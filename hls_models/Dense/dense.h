...
namespace dcnn {
  ...
  namespace cpp {
    ...
    template<typename dtype_I, typename dtype_O,
            int N, int M, int Tm, int Tn, Activation ACT=linear>
    class Dense {
    private:
          ...
          OMAP: for (int m = 0; m < M; m+=Tm) {
            _TIM: for (int tn = 0; tn < Tn; tn++) {
              _TOM: for (int tm = 0; tm < Tm; tm++) {
                ...
                // if (n+tn == N-1) dout[m+tm] = activation(acc[m+tm]);
              } // (loop) _TOM
            } // (loop) _TIM
          } // (loop) OMAP      
        } // (loop) IMAP
  ...
  namespace hls {
    ...
    #pragma hls_design
    template<typename dtype_I, typename dtype_O,
            int N, int M, int Tm, int Tn, Activation ACT=linear, int Accuracy=1>
    class Dense {
    private:
      ...
      typedef ac_channel<wtype>        chanW;
      typedef ac_channel<btype>        chanB;

      ndmatrix::Mat2d<wtype, N, M> weights; // change dimension to follow loops
      ndmatrix::Mat1d<btype, M>    bias; 

      typedef ac_fixed<21,1,false> LUT_t;

      LUT_t exp_L1[Accuracy*128];
      LUT_t exp_L2[8];

      dtype_O sfm_max;
      dtype_O sfm_sum;

      // dtype_I exp(dtype_I &in){
      //   int7bit ind7;
      //   int3bit ind3;
      ...
      // void softmax_max_sum(chanO &vec) {
      //   sfm_max = vec[0];
      //   for (int i = 0; i < M; i++) {
      //     sfm_max = (sfm_max > vec[i]) ? sfm_max : vec[i];
      //   }
      //   sfm_sum = 0;
      //   for (int i = 0; i < M; i++) {
      //     sfm_sum += std::exp(vec[i]-sfm_max);
      ...
      dtype_O activation(dtype_I x) {
        ...
        switch (ACT) {
          ...
          case softmax: { 
            res = (dtype_O)x; // TODO: write softmax
            break;
          }
          default: {
            res = (x > 0) ? (dtype_O)x : (dtype_O)0;
    ...
    public:
      ...
      void CCS_BLOCK(run)(chanL &l, chanW &w, chanB &b, chanI &din, chanO &dout) {
        ...
        if (load_enable) {
          ...
          ndmatrix::Mat1d<dtype_O, M> outBuffer;

          IMAP: for (int n = 0; n < N; n+=Tn) {
            bool write_out = (n >= N-Tn);
            ...
            if (din.available(1)) {
              ...
              OMAP: for (int m = 0; m < M; m+=Tm) {
                ...
                _TIM: for (int tn = 0; tn < Tn; tn++) {
                  _TOM: for (int tm = 0; tm < Tm; tm++) {
                    outBuffer[m+tm] += inp[tn] * weights[n+tn][m+tm];
                    if (write_out) {
                      dtype_I res = outBuffer[m+tm] + bias[m+tm];
                      out[tm] = activation(res);
                    } 
                  } // (loop) _TOM
                } // (loop) _TIM
                if (write_out) 
